...
function AdminDashboardPage() {
    ...
    const miembrosAsignadosProyecto = useMemo(() => {
        return detalleProyecto?.miembrosAsignados || [];
    }, [detalleProyecto]);

    const proyectoNoPermiteNuevasTareas = useMemo(() => {
        return ["COMPLETED", "CANCELLED"].includes(detalleProyecto?.proyecto?.estado);
    }, [detalleProyecto]);
    ...
    const crearTarea = async (event) => {
        ...
        if (formTarea.descripcion.trim().length < 5) {
            setError("Completa la descripción de la tarea.");
            return;
        }
                                                ...
                                                <section className="panel admin-panel form-panel-pro">
                                                    ...
                                                    <form className="form-grid form-grid-pro" onSubmit={crearTarea} noValidate>
                                                        <FormField label="Descripción" help="Describe el trabajo a realizar." className="span-2">
                                                            <textarea name="descripcion" value={formTarea.descripcion} onChange={actualizarCampoTarea} maxLength="500" rows="4" required disabled={proyectoNoPermiteNuevasTareas} />
                                                        </FormField>

                                                        <FormField label="Responsable" help="Solo aparecen miembros asignados al proyecto.">
                                                            <select
                                                                name="responsable"
                                                        ...
                                                        <FormField label="Estado" help="Estado inicial de la tarea.">
                                                            <select name="estado" value={formTarea.estado} onChange={actualizarCampoTarea} disabled={proyectoNoPermiteNuevasTareas}>
                                                        ...
                                                        <FormField label="Fecha de inicio" help="Opcional.">
                                                            <input type="date" name="fechaInicio" value={formTarea.fechaInicio} onChange={actualizarCampoTarea} disabled={proyectoNoPermiteNuevasTareas} />
                                                        </FormField>

                                                        <FormField label="Fin estimado" help="Opcional, pero debe ser posterior al inicio.">
                                                            <input type="date" name="fechaFinEstimada" value={formTarea.fechaFinEstimada} onChange={actualizarCampoTarea} disabled={proyectoNoPermiteNuevasTareas} />
                                                        </FormField>

                                                        <div className="form-actions span-2 form-actions-pro">
                                                            <button className="button secondary" type="button" onClick={() => setFormTarea(tareaInicial)} disabled={guardando || proyectoNoPermiteNuevasTareas}>Limpiar</button>
                                                            <button className="button" type="submit" disabled={guardando || proyectoNoPermiteNuevasTareas}>{guardando ? "Guardando..." : "Crear tarea"}</button>
                                                        </div>
                                                    </form>
                                                </section>
